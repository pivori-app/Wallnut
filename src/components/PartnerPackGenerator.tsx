    ...
    <div className="text-left animate-in fade-in zoom-in duration-300 w-full min-w-full">
      <div className="flex border-b border-gray-200 dark:border-white/10 mb-6 overflow-x-auto no-scrollbar">
        <button 
          ...
          className={`flex-1 py-3 px-4 font-medium text-center border-b-2 transition-colors whitespace-nowrap ${activeTab === 'generate' ? 'border-secondary text-secondary' : 'border-transparent text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}
        >
          Générer (Accord, Pack)
        </button>
        <button 
          onClick={() => setActiveTab('upload')} 
          className={`flex-1 py-3 px-4 font-medium text-center border-b-2 transition-colors whitespace-nowrap ${activeTab === 'upload' ? 'border-secondary text-secondary' : 'border-transparent text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}
        >
          Intégrer PJ
        </button>
        <button 
          onClick={() => setActiveTab('documentation')} 
          className={`flex-1 py-3 px-4 font-medium text-center border-b-2 transition-colors whitespace-nowrap ${activeTab === 'documentation' ? 'border-secondary text-secondary' : 'border-transparent text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}
        >
      ...
      {activeTab === 'generate' && (
        <>
          <div className="flex items-center justify-center gap-4 mb-8">
            <div className={`flex items-center justify-center w-8 h-8 rounded-full font-bold ${step >= 1 ? 'bg-secondary text-white' : 'bg-gray-200 dark:bg-white/10 text-gray-500 dark:text-white/50'}`}>1</div>
            ...
            <div className={`flex items-center justify-center w-8 h-8 rounded-full font-bold ${step >= 2 ? 'bg-secondary text-white' : 'bg-gray-200 dark:bg-white/10 text-gray-500 dark:text-white/50'}`}>2</div>
            <div className="h-0.5 bg-gray-200 dark:bg-white/10 w-24 rounded-full">
              <div className={`h-full bg-secondary rounded-full transition-all ${step >= 3 ? 'w-full' : 'w-0'}`} />
            </div>
            <div className={`flex items-center justify-center w-8 h-8 rounded-full font-bold ${step >= 3 ? 'bg-secondary text-white' : 'bg-gray-200 dark:bg-white/10 text-gray-500 dark:text-white/50'}`}>3</div>
          </div>

          {step === 1 && (
            <div className="space-y-6">
              <h3 className="text-xl font-medium text-gray-900 dark:text-white">Sélectionner un dossier</h3>
              <p className="text-gray-500 dark:text-white/50 text-sm">Sélectionnez le dossier client pour lequel générer le Pack Partenaire d'accord de principe.</p>
              
              <div className="space-y-2 max-h-60 overflow-y-auto pr-2 no-scrollbar">
                {properties.map(p => (
                  <label 
                    key={p.id} 
                    className={`flex items-center justify-between p-4 rounded-xl border cursor-pointer transition-all ${selectedPropId === p.id ? 'border-secondary bg-secondary/10' : 'border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 hover:bg-gray-50 dark:hover:bg-white/10 shadow-sm dark:shadow-none'}`}
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-gray-500 dark:text-white/40 text-[10px] uppercase tracking-wider">{p.referenceNumber} • {p.city}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <div className="text-gray-500 dark:text-white/40 text-xs text-right mt-1">{p.type}</div>
                    </div>
                  </label>
                ))}
                {properties.length === 0 && <div className="text-gray-500 dark:text-white/50 text-center py-4">Aucun dossier disponible.</div>}
              </div>
          ...
          {step === 2 && property && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 bg-white dark:bg-white/5 p-4 rounded-xl border border-gray-200 dark:border-white/10 text-sm shadow-sm dark:shadow-none">
                <div className="space-y-3">
                  <div>
                    <div className="text-gray-400 dark:text-white/40 text-[10px] uppercase font-bold tracking-wider">Vendeur</div>
                    <div className="text-gray-900 dark:text-white font-medium">{property.clientName}</div>
                  </div>
                  <div>
                    <div className="text-gray-400 dark:text-white/40 text-[10px] uppercase font-bold tracking-wider">Adresse complète</div>
                ...
                <div className="space-y-3">
                  <div>
                    <div className="text-gray-400 dark:text-white/40 text-[10px] uppercase font-bold tracking-wider">Ref Dossier</div>
                    <div className="text-gray-900 dark:text-white font-medium">{property.referenceNumber}</div>
                  </div>
                  <div>
                    <div className="text-gray-400 dark:text-white/40 text-[10px] uppercase font-bold tracking-wider">Statut Document</div>
              ...
              <div className="pt-4 flex justify-between items-center">
                <button onClick={() => setStep(1)} className="px-4 py-2 text-sm text-gray-500 dark:text-white/50 font-bold hover:text-gray-900 dark:hover:text-white transition-all">Retour</button>
                <button 
          ...
          {step === 3 && (
            <div className="text-center py-8 space-y-6">
              ...
              <div>
                ...
                <p className="text-gray-500 dark:text-white/50 text-sm">Le "Pack Accord de Principe" est intégré au dossier.</p>
              </div>
              
              <div className="flex flex-col sm:flex-row justify-center gap-4 pt-4 px-8">
                <button onClick={() => { setActiveTab('upload'); setStep(1); }} className="px-4 py-2 border border-gray-300 dark:border-white/20 bg-gray-50 dark:bg-white/5 text-gray-900 dark:text-white font-bold rounded-xl hover:bg-gray-100 dark:hover:bg-white/10 transition-all flex items-center justify-center gap-2 text-sm">
              ...
              <button onClick={onClose} className="mt-8 text-gray-500 dark:text-white/40 text-sm hover:text-gray-900 dark:hover:text-white transition-colors underline underline-offset-4">Fermer sans télécharger</button>
            </div>
          )}
        </>
      )}
      ...
      {activeTab === 'upload' && (
        <div className="space-y-8">
          <div className="text-center">
            ...
            <p className="text-gray-500 dark:text-white/50 text-sm">Intégrez la documentation transmise (pièces d'identité, actes de propriété, baux en cours...).</p>
          </div>

          <div className="border-2 border-dashed border-gray-300 dark:border-white/20 rounded-2xl p-10 flex flex-col items-center justify-center text-center bg-gray-50 dark:bg-white/5 hover:bg-gray-100 dark:hover:bg-white/10 transition-colors cursor-pointer group">
            ...
            <p className="text-xs text-gray-500 dark:text-white/40 mt-2">Formats supportés: PDF, JPG, PNG (Max 15 MB)</p>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                {uploadedFiles.map((f, i) => (
                  <div key={i} className="flex items-center justify-between p-4 bg-white dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10 shadow-sm">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-gray-500 dark:text-white/40 uppercase">Intégré le {f.date}</div>
                      </div>
                    </div>
      ...
      {activeTab === 'documentation' && (
        <div className="space-y-6">
          <div className="mb-6">
            ...
            <p className="text-gray-500 dark:text-white/50 text-sm">Retrouvez l'ensemble des PDF de présentation et la documentation institutionnelle Wallnut.</p>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[
              ...
              <a 
                ...
                className="flex flex-col p-5 bg-white dark:bg-white/5 rounded-2xl border border-gray-200 dark:border-white/10 hover:border-secondary/50 dark:hover:border-secondary/50 hover:shadow-lg transition-all group cursor-pointer"
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="text-gray-400 dark:text-white/40 group-hover:text-secondary p-1 transition-colors">
                    <Download size={18} />
                  </div>
                </div>
                <h4 className="font-bold text-gray-900 dark:text-white text-[15px] mb-2 leading-tight">{doc.title}</h4>
                <p className="text-xs text-gray-500 dark:text-white/60 mb-5 leading-relaxed">{doc.desc}</p>
                <div className="mt-auto pt-4 border-t border-gray-100 dark:border-white/5 flex items-center justify-between text-[11px] text-gray-400 dark:text-white/40 font-bold tracking-wider uppercase">
                  <span>Version</span>
