...
export function PropertySynthesisStep({ propertyData, documents, onComplete, onBack }: any) {
  ...
  return (
    <div className="space-y-8">
      {/* Dashboard Synthèse */}
      <div id="synthesis-content" className="glass p-8 md:p-10 rounded-[2.5rem] border border-gray-200 dark:border-white/10 relative overflow-hidden">
        {/* Decorative Blur */}
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-6">
          <div>
            ...
            <div className="inline-flex mt-4 items-center gap-2 px-3 py-1 bg-black/5 rounded-lg text-app-sm font-bold opacity-70">
              ...
              <div className="mt-8 flex flex-col items-center gap-4">
                ...
                <div className="flex flex-wrap items-center justify-center gap-2 text-app-sm mt-2">
                  ...
                  {Object.entries(estimation.multipliersApplied).map(([name, val]: [string, any]) => (
                    <span key={name} className={cn("font-bold px-2 py-0.5 rounded-md text-app-xs", 
                  ...
                  {Object.keys(estimation.multipliersApplied).length === 0 && (
                     <span className="opacity-40 italic text-app-xs">Aucun ajustement</span>
                  )}
                </div>
              </div>
            </>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {/* Déploiement Drive */}
        <div className="glass p-8 rounded-3xl border border-gray-200 dark:border-white/10 text-center flex flex-col items-center justify-center">
          <div className="w-16 h-16 bg-blue-50 text-blue-600 flex items-center justify-center rounded-2xl mb-4 shadow-inner">
            <HardDrive size={32} />
          </div>
          <h3 className="font-bold text-app-md mb-2">Synchronisation Drive</h3>
          <p className="text-app-sm opacity-70 mb-6 max-w-sm">
          ...
          <button 
            ...
            className={cn(
              "px-4 py-2 text-sm w-full max-w-xs rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-md",
              deploySuccess ? "bg-green-500 text-white" : "bg-white border border-gray-200 dark:border-white/10 hover:border-blue-500 hover:text-blue-600"
            )}
          >
        ...
        <div className="glass p-8 rounded-3xl border border-gray-200 dark:border-white/10 text-center flex flex-col items-center justify-center">
          ...
          <h3 className="font-bold text-app-md mb-2">Partage Intelligent (Agrafe)</h3>
          <p className="text-app-sm opacity-70 mb-6 max-w-sm">
            Générer un envoi mail ciblé avec des pièces jointes adaptées au destinataire.
          </p>
          <button 
            onClick={() => setShowShareModal(true)}
            className="px-4 py-2 text-sm w-full max-w-xs rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-md bg-primary text-white hover:-translate-y-1 hover:shadow-lg"
          >
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4 pt-4 border-t border-gray-200 dark:border-white/10">
        ...
        <button 
          onClick={onComplete}
          className="px-6 py-2 text-sm rounded-xl font-bold flex items-center justify-center gap-2 transition-all bg-black text-white shadow-lg hover:shadow-xl hover:-translate-y-1"
        >
          Valider et Quitter <CheckCircle2 size={16} />
        </button>
      </div>
      ...
      {showShareModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[2.5rem] p-8 max-w-md w-full shadow-2xl relative">
            ...
            <h3 className="text-app-xl font-display font-bold mb-2">Composition Smart Share</h3>
            <p className="text-app-sm opacity-60 mb-6">Sélectionnez le destinataire pour adapter automatiquement le contenu du bundle (ID: {bundleId}).</p>
            ...
            <div className="space-y-4 mb-8">
              <button 
                ...
                className={cn(
                  ...
                  shareTarget === 'notaire' ? "border-primary bg-primary/5" : "border-gray-200 dark:border-white/10 hover:border-primary/30"
                )}
              >
                ...
                <div>
                  ...
                  <div className="text-app-xs opacity-70 mt-1">Inclut : Juridique, Technique, Fiscal, Titre de propriété, CNI.</div>
                </div>
              </button>
              ...
              <button 
                ...
                className={cn(
                  "w-full p-4 rounded-2xl border-2 flex items-start gap-4 transition-all text-left",
                  shareTarget === 'agent' ? "border-primary bg-primary/5" : "border-gray-200 dark:border-white/10 hover:border-primary/30"
                )}
              >
                <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm shrink-0 mt-1">
                ...
                <div>
                  ...
                  <div className="text-app-xs opacity-70 mt-1">Inclut : DPE, Photos, Descriptif, Taxe Foncière (masquée partialement).</div>
                </div>
              </button>
            </div>
